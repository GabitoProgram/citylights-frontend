...
const BookingFacturasList: React.FC = () => {
  const [facturas, setFacturas] = useState<FacturaBooking[]>([]);
  ...
  useEffect(() => {
    const fetchFacturas = async () => {
      try {
        const token = localStorage.getItem('access_token');
        const response = await fetch('http://localhost:3000/api/proxy/booking-copia/factura', {
          headers: {
            'Authorization': `Bearer ${token}`,
...
export default BookingFacturasList;
